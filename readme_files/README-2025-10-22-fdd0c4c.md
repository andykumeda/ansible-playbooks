...
### Running Playbooks
- **Full site configuration**: `ansible-playbook -i inventory/hosts playbooks/site.yml`
...
### Inventory Selection
- The default inventory is `inventory/hosts` as configured in `ansible.cfg`.
- Deprecated inventories live under `inventory/old/` (e.g., baxter.yml, vxrail.yml, c9410.yml). They are not used by default.
- Groups available in the active inventory include the top-level composites (e.g. 2e39, hrw, lab, mpoe, vdm) and per-location switches like `2e39_switches`, `hrw_switches`, `lab_switches`, `mpoe_switches`, `vdm_switches` etc.
- To run against a subset of hosts from the active inventory, use `--limit` with group names or host patterns.
- YAML inventories for legacy/configs can be executed by pointing `-i` at the appropriate file, e.g. `inventory/old/vxrail.yml`, but this is not recommended for new tasks.

### Subset Run Examples (active inventory)
- Single group: `ansible-playbook -i inventory/hosts playbooks/site.yml --limit 2e39_switches`
- Multiple groups (union): `ansible-playbook -i inventory/hosts playbooks/site.yml --limit "2e39_switches:hrw_switches"`
- Intersection: `ansible-playbook -i inventory/hosts playbooks/site.yml --limit "2e39_switches&hrw_switches"`
- Subset of hosts in a group by index: `ansible-playbook -i inventory/hosts playbooks/site.yml --limit "2e39_switches[0:3]"`
- Preview targets without executing: `ansible-playbook -i inventory/hosts playbooks/site.yml --list-hosts`

### Advanced
- Combining inventories (advanced/rare): If you need to target devices from multiple inventories explicitly, you can pass multiple sources: `-i "inventory/hosts,inventory/old/vxrail.yml" ...` (not recommended for daily use).
...
### Subset Run Guide
- Overview: Use --limit with group or host patterns to run against a subset of hosts in the active inventory. See examples below.
- Patterns: single group, multiple groups (union), intersection, subset by index, exclude, list-hosts.
- Examples:
  - Run single group: ansible-playbook -i inventory/hosts playbooks/site.yml --limit 2e39_switches
  - Run multiple groups: ansible-playbook -i inventory/hosts playbooks/site.yml --limit "2e39_switches:hrw_switches"
  - Intersection: ansible-playbook -i inventory/hosts playbooks/site.yml --limit "2e39_switches&hrw_switches"
  - Subset by index: ansible-playbook -i inventory/hosts playbooks/site.yml --limit "2e39_switches[0:3]"
  - Preview: ansible-playbook -i inventory/hosts playbooks/site.yml --list-hosts
- Note: This guide complements the existing README guidance; the legacy inventory under inventory/old remains available but is not used by default.
